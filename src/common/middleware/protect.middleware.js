...
const protect = async (req, res, next) => {
   try {
      ...
      if (type !== TOKEN_TYPE) {
         throw new UnAuthorizedException(
            `Authorization header phải có định dạng: ${TOKEN_TYPE} token.`
         );
      }
      if (!accessToken) {
         // Xác minh token có tồn tại/hợp lệ không
      ...
      if (!userExist) {
         throw new UnAuthorizedException(
            `Người dùng ${userExist.nguoi_dung_id} không tồn tại.`
         );
      }
      // gán thông tin user vào req.user để các controller phía sau protect dùng.
      req.user = userExist;
      next();
   } catch (error) {
